...
extends HttpUserAuthentication {
    ...
    @Override public void logIn() throws AuthenticationException {
        if (StringUtils.isBlank(getUsername())) throw new InvalidCredentialsException("Invalid username");
        if (StringUtils.isNotBlank(getAuthenticatedToken())) logInWithToken();
        else if (StringUtils.isNotBlank(getPassword())) logInWithPassword();
    ...
    protected void logInWithPassword() throws AuthenticationException {
        ...
        if (response.getSelectedProfile() != null) setUserType(response.getSelectedProfile().isLegacy() ? UserType.LEGACY : UserType.MOJANG);
        else if (ArrayUtils.isNotEmpty(response.getAvailableProfiles())) setUserType(response.getAvailableProfiles()[0].isLegacy() ? UserType.LEGACY : UserType.MOJANG);

        User user = response.getUser();
        if (user != null && user.getId() != null) setUserid(user.getId());
        else setUserid(getUsername());
    ...
    protected void updateUserProperties(User user) {
        if (user == null) return;
        if (user.getProperties() != null) getModifiableUserProperties().putAll(user.getProperties());
    }

    protected void logInWithToken() throws AuthenticationException {
        if (StringUtils.isBlank(getUserID())) {
            if (StringUtils.isBlank(getUsername())) setUserid(getUsername());
            else throw new InvalidCredentialsException("Invalid uuid & username");
        }

        if (StringUtils.isBlank(getAuthenticatedToken())) throw new InvalidCredentialsException("Invalid access token");
        ...
        if (response.getUser() != null && response.getUser().getId() != null) setUserid(response.getUser().getId());
        else setUserid(getUsername());
    ...
    @Override public void selectGameProfile(GameProfile profile) throws AuthenticationException {
        if (!isLoggedIn()) throw new AuthenticationException("Cannot change game profile whilst not logged in");
        if (getSelectedProfile() != null) throw new AuthenticationException("Cannot change game profile. You must log out and back in.");
        if (profile == null || !ArrayUtils.contains(profiles, profile)) throw new IllegalArgumentException("Invalid profile '" + profile + "'");

        RefreshRequest request = new RefreshRequest(this, profile);
        RefreshResponse response = getAuthenticationService().makeRequest(ROUTE_REFRESH, request, RefreshResponse.class);
        if (!response.getClientToken().equals(getAuthenticationService().getClientToken())) throw new AuthenticationException("Server requested we change our client token. Don't know how to handle this!");
    ...
    @Override public Map<String, Object> saveForStorage() {
        ...
        if (StringUtils.isNotBlank(getAuthenticatedToken())) result.put(STORAGE_KEY_ACCESS_TOKEN, getAuthenticatedToken());

        return result;
    }

    @Deprecated public String getSessionToken() {
        if (isLoggedIn() && getSelectedProfile() != null && canPlayOnline()) return String.format(
            "token:%s:%s",
